      ...
      <tr>
        ...
        <td>
          <div class="avatar-cell">
            @if ($castMember->image)
              <img src="{{ Storage::url($castMember->image) }}" class="avatar-sm" style="object-fit:cover;border-radius:50%">
            @else
...
<div class="modal-overlay" id="cast-member-modal">
  <div class="modal">
    ...
    <form method="POST" action="{{ route('cast-members.store') }}" id="cast-member-form" enctype="multipart/form-data">
      @csrf
...
function resetCastMemberForm() {
  const form = document.getElementById('cast-member-form');
  form.action = '{{ route("cast-members.index") }}';
  form.querySelector('input[name="_method"]')?.remove();
  form.reset();
  form.querySelector('[name="image"]').value = '';
  const cur = form.querySelector('.file-current');
  if (cur) cur.remove();
  document.getElementById('cast-member-modal-title').textContent = 'Tambah Pemeran Baru';
...
function viewCastMember(id) {
  ...
  document.getElementById('view-cast-member-body').innerHTML =
    ...
    '<div><div style="font-size:20px;font-weight:800;margin-bottom:4px">' + c.nama + '</div><div style="font-size:13px;color:var(--text-2)">' + c.film + ' · ' + c.peran + '</div><div style="margin-top:8px"><span class="badge ' + sc + '">' + c.status + '</span></div></div></div>' +
